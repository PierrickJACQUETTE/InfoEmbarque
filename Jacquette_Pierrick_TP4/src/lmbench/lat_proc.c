...
void do_thread(iter_t iterations, void* cookie);
...
main(int ac, char **av)
{
	...
	int repetitions = -1;
	int c;
	char* usage = "[-P <parallelism>] [-W <warmup>] [-N <repetitions>] thread|procedure|fork|exec|shell\n";
	...
	} else if (!strcmp("thread", av[optind])) {
			benchmp(NULL, do_thread, cleanup, 0, parallel, 
			warmup, repetitions, NULL);
		micro(STATIC_PREFIX "Process create+exit", get_n());
...
void 
do_thread(iter_t iterations, void* cookie)
{
	signal(SIGCHLD, SIG_DFL);
	handle_scheduler(benchmp_childid(), 0, 1);
	while (iterations-- > 0) {
		pthread_t thread1;
		pthread_create(&thread1, NULL, threadFonctionCreate, NULL);
